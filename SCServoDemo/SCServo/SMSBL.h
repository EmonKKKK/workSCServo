...
#define SMSBL_MAX_LOAD_L 16//最大输出扭矩
#define	SMSBL_MAX_LOAD_H 17
#define SMSBL_CW_DEAD 26
#define SMSBL_CCW_DEAD 27
#define SMSBL_OFS_L 31
#define SMSBL_OFS_H 32
#define SMSBL_MODE 33
#define SMSBL_PROTECR_LOAD	34//保护扭矩
#define SMSBL_PROTECT_TIME	35//保护时间
#define SMSBL_OVERLOAD		36//过载扭矩
...
public:
	...
	virtual int SetRunMode(u8 ID, u8 Mode);//设置运行模式
	virtual int AngleLimit(u8 ID, u16 Min,u16 Max);//设置角度限制
	virtual int MaxLoad(u8 ID, u16 MaxLoad);//最大扭矩
	virtual int OverLoadProtect(u8 ID, u8 ProtectLoad, u8 ProtectTime, u8 OverLoad);// 设置 保护扭矩 保护时间 过载扭矩
	//virtual int SynOverLoadProtect(u8 ID,);//同步写过载保护保护扭矩 保护时间 过载扭矩
	virtual int WriteAcc(u8 ID, u8 Acc);//设置加速度
