...
// Yanmeng
int SMSBL::SetRunMode(u8 ID , u8 Mode)
{
	return writeByte(ID, SMSBL_MODE, Mode);
}


int SMSBL::AngleLimit(u8 ID, u16 Min, u16 Max)
{
	u8 bBuf[2];
	Host2SCS(bBuf + 0, bBuf + 1, Min);
	genWrite(ID, SMSBL_MIN_ANGLE_LIMIT_L, bBuf, 2);
	Host2SCS(bBuf + 0, bBuf + 1, Max);
	return genWrite(ID, SMSBL_MAX_ANGLE_LIMIT_L, bBuf, 2);

}

int SMSBL::MaxLoad(u8 ID, u16 MaxLoad)
{
	u8 bBuf[2];
	Host2SCS(bBuf + 0, bBuf + 1, MaxLoad);
	return genWrite(ID, SMSBL_MAX_LOAD_L, bBuf, 2);
}


int SMSBL::OverLoadProtect(u8 ID, u8 ProtectLoad, u8 ProtectTime, u8 OverLoad)
{
	u8 bBuf[3];
	bBuf[0] = ProtectLoad;
	bBuf[1] = ProtectTime;
	bBuf[2] = OverLoad;
	return genWrite(ID, SMSBL_PROTECR_LOAD, bBuf, 3);//因为地址是连着的，所以一起配置


}

int SMSBL::WriteAcc(u8 ID, u8 Acc)
{
	return writeByte(ID, SMSBL_ACC, Acc);
}

int SMSBL::WriteSpeed(u8 ID, u16 Speed)
{
	u8 bBuf[2];
	Host2SCS(bBuf + 0, bBuf + 1, Speed);
	return genWrite(ID, SMSBL_GOAL_SPEED_L, bBuf, 2);
...
int SMSBL::WritePositon(u8 ID, u16 Position)
{
	u8 bBuf[2];
	Host2SCS(bBuf + 0, bBuf + 1, Position);
	return genWrite(ID, SMSBL_GOAL_POSITION_L, bBuf, 2);
}

int SMSBL::ReadIsMove(int ID)
{
	int IsMove = -1;
	if (ID == -1) {
		IsMove = Mem[SMSBL_MOVING - SMSBL_PRESENT_POSITION_L];
	}
	else {
		Err = 0;
		IsMove = readByte(ID, SMSBL_MOVING);
		if (IsMove == -1) {
			Err = 1;
		}
	}
